...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        // 如果使用的是液体玻璃，则需要扩展 _UISearchBarVisualProviderIOS 的 setUpSearchField 方法，
        // 在 setUpSearchField 方法中，需要设置搜索框的样式，包括搜索框的背景颜色、搜索框的边框颜色、搜索框的文字颜色等。
        // 如果使用的是液体玻璃，则需要扩展 _UISearchBarVisualProviderIOS 的 setUpSearchField 方法，
        // 在 setUpSearchField 方法中，需要设置搜索框的样式，包括搜索框的背景颜色、搜索框的边框颜色、搜索框的文字颜色等。
...
- (UIButton *)qmui_cancelButton {
    // 如果使用的是液体玻璃，则需要返回 searchTextField 的 _clearButton
    if (QMUIHelper.isUsedLiquidGlass) {
